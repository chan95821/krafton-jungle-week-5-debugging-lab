...
static void directory_sort_by_name(Directory *d) {
    for (int i = 0; i < d->count; i++) {
        for (int j = i + 1; j < d->count; j++) { // selection sort
...
static void directory_free(Directory *d) {
    ...
    // for (int i = 0; i < d->count; i++) {
    //     free(d->by_name[i]);        // i == 0 일 때 double free err.    : 
        // by_name이 by_idx와 같은 포인터로 가지고 있으니, double free일 수 밖에 없다.    
        // 이미 해제 되었으므로, 굳이 할 필요 없다.
    // }
    d->count = 0;
}
...
int main(void) {
    ...
    directory_free(&dir);     // 여기서 free() 시 오류              
    printf("done\n");
    return 0;
}
